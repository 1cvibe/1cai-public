...
class SelfEvolvingMetrics(BaseModel):
    ...
    performance_score: float = Field(default=0.0, ge=0.0, le=100.0, description="Performance score (0-100)")
    last_evolution: Optional[datetime] = Field(default=None, description="Last evolution timestamp")
...
class SelfHealingMetrics(BaseModel):
    ...
    success_rate: float = Field(default=0.0, ge=0.0, le=100.0, description="Healing success rate (0-100)")
    avg_healing_time_ms: float = Field(default=0.0, description="Average healing time in milliseconds")
...
class DistributedAgentMetrics(BaseModel):
    ...
    avg_consensus_time_ms: float = Field(default=0.0, description="Average consensus time in milliseconds")


class CodeDNAMetrics(BaseModel):
    """Code DNA System metrics"""
    generations: int = Field(default=0, description="Number of generations")
    fitness_score: float = Field(default=0.0, ge=0.0, le=100.0, description="Fitness score (0-100)")
...
class PredictiveGenerationMetrics(BaseModel):
    ...
    accuracy: float = Field(default=0.0, ge=0.0, le=100.0, description="Prediction accuracy (0-100)")
    avg_lead_time_hours: float = Field(default=0.0, description="Average lead time in hours")


class RevolutionaryComponentState(BaseModel):
    """State of a revolutionary component"""
    name: str = Field(..., description="Component name")
    status: ComponentStatus = Field(default=ComponentStatus.INITIALIZING, description="Component status")
...
class RevolutionaryOrchestratorState(BaseModel):
    ...
    components: List[RevolutionaryComponentState] = Field(default_factory=list, description="Component states")
    total_enabled: int = Field(default=0, description="Number of enabled components")
    overall_health: float = Field(default=0.0, ge=0.0, le=100.0, description="Overall health score (0-100)")
    started_at: datetime = Field(default_factory=datetime.utcnow, description="Orchestrator start time")
