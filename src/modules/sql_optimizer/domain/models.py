...
class QueryAnalysis(BaseModel):
    ...
    missing_indexes: List[str] = Field(
        ...
        description="Отсутствующие индексы"
    )
    full_table_scans: int = Field(
        default=0,
        ge=0,
        description="Полные сканирования таблиц"
    )
...
class OptimizedQuery(BaseModel):
    ...
    estimated_speedup: float = Field(
        ...,
        ge=1.0,
        description="Оценка ускорения (раз)"
    )
...
class OptimizationResult(BaseModel):
    ...
    index_recommendations: List[IndexRecommendation] = Field(
        default_factory=list,
        description="Рекомендации по индексам"
    )
    optimized_query: Optional[OptimizedQuery] = Field(
        default=None,
        description="Оптимизированный запрос"
    )
    performance_prediction: Optional[PerformancePrediction] = Field(
        default=None,
        description="Предсказание производительности"
    )
