...
class QueryAnalyzer:
    ...
    async def analyze_query(
        ...
        try:
            ...
            complexity = self._analyze_complexity(query.query_text)

            # Detect issues
            issues = self._detect_issues(query.query_text)

            # Find missing indexes
            missing_indexes = self._find_missing_indexes(query.query_text)

            # Count full table scans
            full_table_scans = self._count_full_table_scans(
                query.query_text
            )
    ...
    def _detect_issues(self, query_text: str) -> List[str]:
        ...
        # Function in WHERE
        if re.search(r'where.*\b(upper|lower|substring|trim)\s*\(', query_lower):
            issues.append("Function in WHERE clause prevents index usage")

        # OR in WHERE
        or_count = len(re.findall(r'\bor\b', query_lower))
        if or_count > 3:
            issues.append(f"Multiple OR conditions ({or_count}) - consider using IN")
