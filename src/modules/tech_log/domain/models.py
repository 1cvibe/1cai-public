...
class PerformanceIssue(BaseModel):
    ...
    auto_fix_available: bool = Field(
        default=False,
        description="Доступен auto-fix"
    )
...
class LogAnalysisResult(BaseModel):
    ...
    events_by_type: dict = Field(
        ...
        description="События по типам"
    )
    events_by_severity: dict = Field(
        default_factory=dict,
        description="События по severity"
    )
...
class PerformanceAnalysisResult(BaseModel):
    ...
    performance_issues: List[PerformanceIssue] = Field(
        ...
        description="Проблемы производительности"
    )
    top_slow_queries: List[dict] = Field(
        default_factory=list,
        description="Топ медленных запросов"
    )
    top_slow_methods: List[dict] = Field(
        default_factory=list,
        description="Топ медленных методов"
    )
    errors_by_type: dict = Field(
        default_factory=dict,
        description="Ошибки по типам"
    )
    locks_analysis: dict = Field(
        default_factory=dict,
        description="Анализ блокировок"
    )
