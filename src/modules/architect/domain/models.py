...
class ArchitectureMetrics(BaseModel):
    ...
    coupling_score: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Coupling score (0-1, lower is better)"
    )
    cohesion_score: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Cohesion score (0-1, higher is better)"
    )
    cyclic_dependencies_count: int = Field(
        ...,
        ge=0,
        description="Количество циклических зависимостей"
    )
    god_objects_count: int = Field(
        ...,
        ge=0,
        description="Количество God Objects"
    )
    orphan_modules_count: int = Field(
        ...,
        ge=0,
        description="Количество изолированных модулей"
    )
    overall_score: float = Field(
        ...,
        ge=1.0,
        le=10.0,
        description="Общая оценка архитектуры (1-10)"
    )
...
class AntiPattern(BaseModel):
    ...
    estimated_days: int = Field(
        ...,
        ge=1,
        description="Оценка времени (дни)"
    )
...
class ArchitectureAnalysisResult(BaseModel):
    ...
    anti_patterns: List[AntiPattern] = Field(
        ...
        description="Обнаруженные anti-patterns"
    )
...
class Consequences(BaseModel):
    ...
    positive: List[str] = Field(
        default_factory=list,
        description="Позитивные последствия"
    )
    negative: List[str] = Field(
        default_factory=list,
        description="Негативные последствия"
    )
...
class ADR(BaseModel):
    ...
    alternatives: List[Alternative] = Field(
        default_factory=list,
        description="Рассмотренные альтернативы"
    )
