...
try:
    ...
    # Metrics
    cache_hits_total = Counter("cache_hits_total", "Total cache hits", ["layer"])
    ...
    cache_size = Gauge("cache_size", "Current cache size", ["layer"])
except ImportError:
...
class MultiLayerCache:
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        if value is not None:
            ...
            logger.debug("Cache L1 HIT", extra={"key": key})
            return value
        ...
        if self.redis and self.redis_circuit_breaker.can_attempt():
            try:
                ...
                if cached_value:
                    ...
                    logger.debug("Cache L2 HIT (Redis)", extra={"key": key})
    ...
    async def set(self, key: str, value: Any, ttl_seconds: int = 3600, tags: List[str] = None):
        """
        ...
        if self.redis and self.redis_circuit_breaker.can_attempt():
            try:
                ...
                logger.warning("Redis timeout when setting key", extra={"key": key})
                self.redis_circuit_breaker.record_failure()
            except Exception as e:
                logger.error(
                    "Redis set error",
    ...
    async def invalidate(self, key: str):
        ...
        if self.redis and self.redis_circuit_breaker.can_attempt():
            try:
                await asyncio.wait_for(self.redis.delete(key), timeout=1.0)
                self.redis_circuit_breaker.record_success()
            except Exception as e:
                logger.error(
                    "Redis delete error",
