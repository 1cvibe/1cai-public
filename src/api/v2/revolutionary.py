...
class BatchEvolveRequest(BaseModel):
    ...
    iterations: int = Field(default=1, ge=1, le=10, description="Number of evolution iterations")
...
class BatchEvolveResponse(BaseModel):
    ...
    results: List[Dict[str, Any]] = Field(default_factory=list, description="Results if sync")
    status: str = Field(description="Status")


class HealBatchRequest(BaseModel):
    """Batch healing request"""
    code_snippets: List[str] = Field(..., min_items=1, max_items=10, description="Code to heal")
