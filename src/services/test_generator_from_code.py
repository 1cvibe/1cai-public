...
class CodeToTestGenerator:
    ...
    def _guess_parameter_type(self, param_name: str) -> str:
        ...
        if any(word in param_lower for word in ["сумма", "количество", "число", "count", "amount"]):
            return "Число"
        elif any(word in param_lower for word in ["строка", "текст", "имя", "string", "text", "name"]):
            return "Строка"
        elif any(word in param_lower for word in ["дата", "date", "время", "time"]):
            return "Дата"
        elif any(word in param_lower for word in ["список", "массив", "массив", "list", "array"]):
            return "Массив"
...
"""
    ...
    def _generate_assert_code(self, return_type: Optional[str], variable: str) -> str:
        """Генерирует код проверки."""
        if return_type == "Число":
            return f"""    ЮТест.ОжидаетЧто({variable}, "Результат функции")
        .Заполнено()
        .ИмеетТип("Число");"""
        elif return_type == "Строка":
            return f"""    ЮТест.ОжидаетЧто({variable}, "Результат функции")
        .Заполнено()
        .ИмеетТип("Строка")
        .ИмеетДлинуБольше(0);"""
        elif return_type == "Булево":
            return f"""    ЮТест.ОжидаетЧто({variable}, "Результат функции")
        .ИмеетТип("Булево")
        .ЭтоНеНеопределено();"""
        else:
    ...
    def _generate_edge_case_tests(self, func_info: FunctionInfo) -> List[str]:
        ...
        if any(p["type"] == "Число" for p in func_info.parameters):
            tests.append(f"""// Edge case: нулевые значения
Процедура Тест_{func_info.name}_НулевыеЗначения() Экспорт
    
    // Arrange
    {', '.join(f"{p['name']} = 0" if p['type'] == 'Число' else f"{p['name']} = Неопределено" for p in func_info.parameters)};
...
Процедура Тест_{func_info.name}_МаксимальныеЗначения() Экспорт
    ...
    // Arrange
    {', '.join(f"{p['name']} = 999999999" if p['type'] == 'Число' else f"{p['name']} = \"{'A' * 1000}\"" if p['type'] == 'Строка' else f"{p['name']} = Неопределено" for p in func_info.parameters)};
...
""")
    ...
    def _generate_negative_tests(self, func_info: FunctionInfo) -> List[str]:
        ...
        tests.append(f"""// Негативный тест: неопределенные параметры
Процедура Тест_{func_info.name}_НеопределенныеПараметры() Экспорт
    
    // Arrange
    {', '.join(f"{p['name']} = Неопределено" for p in func_info.parameters)};
