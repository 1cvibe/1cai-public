...
class TCPOptimizer:
    ...
    def _apply_tcp_keepalive(self):
        ...
        subprocess.run(
            ["sysctl", "-w", f"net.ipv4.tcp_keepalive_time={self.config.tcp_keepalive_time}"],
            check=True
        )
        subprocess.run(
            ["sysctl", "-w", f"net.ipv4.tcp_keepalive_probes={self.config.tcp_keepalive_probes}"],
            check=True
        )
        subprocess.run(
            ["sysctl", "-w", f"net.ipv4.tcp_keepalive_intvl={self.config.tcp_keepalive_intvl}"],
            check=True
        )
    ...
    def _apply_tcp_max_syn_backlog(self):
        ...
        subprocess.run(
            ["sysctl", "-w", f"net.ipv4.tcp_max_syn_backlog={self.config.tcp_max_syn_backlog}"],
            check=True
        )
    ...
    def _apply_tcp_synack_retries(self):
        ...
        subprocess.run(
            ["sysctl", "-w", f"net.ipv4.tcp_synack_retries={self.config.tcp_synack_retries}"],
            check=True
        )
