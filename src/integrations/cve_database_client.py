...
class CVEDatabaseClient:
    ...
    def _init_clients(self):
        ...
        if self.nvd_api_key:
            try:
                ...
                self.logger.error(f"Failed to init NVD client: {e}")
        ...
        if self.snyk_api_key:
            try:
                ...
                self.logger.error(f"Failed to init Snyk client: {e}")
        ...
        try:
            ...
            self.logger.error(f"Failed to init OSV client: {e}")
    ...
    async def _check_nvd(
        ...
        self.logger.debug(f"Checking NVD for {package_name}@{version}")
        return []
    ...
    async def _check_snyk(
        ...
        # https://snyk.io/api/
        self.logger.debug(f"Checking Snyk for {package_name}@{version}")
        return []
    ...
    async def _check_osv(
        ...
        # https://osv.dev/
        self.logger.debug(f"Checking OSV for {package_name}@{version}")
        return []
    ...
    async def get_cve_details(self, cve_id: str) -> Optional[CVE]:
        ...
        for source_func in sources:
            try:
                ...
                self.logger.debug(f"Source failed for {cve_id}: {e}")
                continue
