...
| **CI/CD и качество** | Ступенчатые пайплайны, build once → promote, smoke после деплоя, отчёты | CI покрывает тесты, docs lint, spec validation; релизы автоматизированы (`release.yml`); smoke-tests компилируют модули и дергают `/health`; HTML/JUnit/Allure артефакты | Подготовить smoke job с поднятием минимального сервиса; внедрить Allure отчёты на другие пакеты |
...
| **Observability & Monitoring** | RED/USE метрики, логи, трассировки, SLO/Error Budgets, alert runbooks | `github-monitor.yml`, `dora-metrics.yml`, `/metrics` (Prometheus Instrumentator), `docs/observability/SLO.md`, `docs/runbooks/alert_slo_runbook.md` | Добавить сбор SLI (Prometheus exporter/Logs), завести alert-канал |
