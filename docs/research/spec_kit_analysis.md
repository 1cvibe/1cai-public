...
| **Security (DevSecOps)** | Shift-left (SAST/DAST), dependency & secret scanning, policy-as-code | Bandit, Safety, run_full_audit; workflows `secret-scan.yml` (Gitleaks) и `trufflehog.yml` (Trufflehog); Conftest Rego (`policy/kubernetes/*.rego`), Semgrep (`security/semgrep.yml`), Checkov/Trivy (`scripts/security/run_checkov.sh`), make `policy-check`, `preflight`; Vault best practices (`docs/ops/vault.md`) | Расширить конституцию пунктами по least privilege; рассмотреть OPA/policy-as-code для Terraform/GitOps |
| **Feedback & Continuous Improvement** | Метрики delivery (lead time, deploy freq, MTTR, CFR), UX feedback, DevEx | Скрипт `collect_dora.py`, workflow `dora-metrics.yml`, AWS Cost report (`scripts/finops/aws_cost_report.py`), preflight checklist (`scripts/checklists/preflight.sh`); ретроспективы не описаны | Добавить визуализацию/дашборд, включить DORA summary в README/ретро; описать feedback loop |
