...
> - чтобы убедиться, что схемы свежие, выполните `make render-uml` (под капотом [`scripts/docs/render_uml.py`](scripts/docs/render_uml.py); используется тот же скрипт, что и в CI);
